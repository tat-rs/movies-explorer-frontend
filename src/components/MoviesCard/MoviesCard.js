...
function MoviesCard({
  ...
  return (
    <li className="card">
      <img className="card__image" src={`https://api.nomoreparties.co${image}`} alt={nameRU}></img>
      <div className="card__container">
        <h3 className="card__title">{nameRU}</h3>
        <Switch>
          <Route path="/movies">
            <button className="card__button button" type="button">
              <svg className="card__icon-like" viewBox="0 0 10 9" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <p className="card__duration">{getTime(duration)}</p>
    </li>
  )
}
